...
export async function getdata() {
  ...
  return data.map((post) => ({
    ...post,
    cat: post.cat.split(","), // Assuming 'cat' is comma-separated categories
  }));
}

export default async function Posts({ findpost }) {
  const dataposts = await getdata();
  const separatecat = findpost && findpost.cat.split(",");

  // Get the last 3 posts
  const lastThreePosts = findpost
    ? dataposts
        .filter(
          (items) =>
            items.cat.some((item) =>
              separatecat.some((item2) => item == item2)
            ) && items.h1title !== findpost.h1title
        )
        .slice(-3)
    : dataposts.slice(-3);
const lastThreePostsnew=dataposts.slice(-3)
  return (
    <div className="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
      <div className="max-w-2xl mx-auto text-center mb-10 lg:mb-14">
        <h3 className="text-2xl font-bold md:text-4xl md:leading-tight">
          {lastThreePosts.length > 0 && findpost
            ? "مقالات مرتبط"
            : "آخرین پست ها"}
        </h3>
        <p className="mt-1 text-gray-600">
          جدیدترین{" "}
          {lastThreePosts.length > 0 && findpost ? " مقالات مرتبط" : "پست ها"}{" "}
          را میتوانید در اینجا مشاهده نمایید.
        </p>
      </div>
